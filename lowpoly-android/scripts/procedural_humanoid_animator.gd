...
func update_animation(delta: float, horizontal_speed: float, vertical_velocity: float, on_floor: bool) -> void:
	...
	_apply_animation_pose(vertical_velocity, on_floor)

func _apply_animation_pose(vertical_velocity: float, on_floor: bool) -> void:
	...
	var spine_pitch: float = -0.08 * run_weight + breathing * 0.018 * (1.0 - movement_blend)
	spine_pitch = lerp(spine_pitch, -0.06, jump_weight)
	_set_rotation("spine", Vector3(spine_pitch, -hips_yaw * 0.35, 0.0))
	_set_rotation("chest", Vector3(breathing * 0.012 * (1.0 - movement_blend), -hips_yaw * 0.25, 0.0))
	_set_rotation("neck", Vector3(-spine_pitch * 0.20, 0.0, 0.0))
	_set_rotation("head", Vector3(-spine_pitch * 0.15, 0.0, 0.0))

	var left_arm_swing: float = opposite_stride * 0.38 * run_weight
	...
	left_elbow = lerp(left_elbow, 0.28, jump_weight)
	right_elbow = lerp(right_elbow, 0.28, jump_weight)
	_set_rotation("left_forearm", Vector3(left_elbow, 0.0, 0.0))
	_set_rotation("right_forearm", Vector3(right_elbow, 0.0, 0.0))

	var left_leg_swing: float = stride * 0.42 * run_weight
	var right_leg_swing: float = opposite_stride * 0.42 * run_weight
	var jump_thigh: float = 0.18 if vertical_velocity >= -0.2 else 0.10
	...
	var left_knee: float = max(0.0, -stride) * 0.45 * run_weight
	var right_knee: float = max(0.0, -opposite_stride) * 0.45 * run_weight
	var jump_knee: float = 0.32 if not on_floor else 0.0
